...
rpc conn method = do
	let msg = BL.toStrict $ pack $ ObjectArray [ObjectStr "rustorion-server-0", ObjectStr method, ObjectBin $ toStrict $ pack $ ObjectArray []]
...
setActions conn acts = rpc conn "set_actions" >> pure ()
